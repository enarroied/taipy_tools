...
def convert_video_to_gif(config: VideoToGifConfig) -> str:
    """
    Convert video to GIF.

    Args:
        config (VideoToGifConfig): Configuration for video-to-GIF conversion

    Returns:
        str: Path to the generated GIF file
    """
    try:
        _validate_input_file(config.input_path)
        ...
        clip_info = _get_clip_info(config.input_path)
        video_duration = clip_info["duration"]
        _validate_parameters(config.start_time, config.duration, video_duration)

        _convert_video_to_gif(config, output_path, clip_info)
        return output_path
...
def _convert_video_to_gif(
    ...
    _log_results(config.input_path, clip_info, config.fps)
    palette_path = _generate_palette(
        config.input_path, config.start_time, config.duration, config.resize_factor
    )
    _create_gif(
        config.input_path,
        output_path,
